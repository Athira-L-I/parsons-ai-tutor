...
export const convertParsonsGraderFuncToEnum = (
  grader?: (() => void) | string | undefined
): ParsonsGrader => {
  if (!grader) {
...
const parseUnitTestArguments = (str: string): AssertEqualParams => {
  ...
  unitTestChecks.forEach((check: string) => {
    ...
    errorMessage.push(
      expArgumentsArr[2] ? check.slice(...expArgumentsArr[2].range) : ''
    );
  });
  return {
...
const parseUnitTests = (unitTestsStr: string | undefined): string[] => {
  ...
  return Array.from(testMatches);
};

export const convertUnitTestsFromString = (
  unitTestsStr: string | undefined
): UnitTest[] => {
  const unitTests = parseUnitTests(unitTestsStr);
  const unitTestsArr: UnitTest[] = [];

  if (unitTests.length === 0) return unitTestsArr;

  unitTests.forEach((test) => {
    const pattern =
      /\s*def\s*(.*?)\(.*?\):\n^(\s*self\.assertEqual\(.*?\))*\s*$/m;
    const checks: RegExpExecArray | null = pattern.exec(test);
    if (checks && checks.length > 0) {
      const firstAssertEqualIndex = checks[0].indexOf('self.assertEqual');
      unitTestsArr.push({
        name: checks[1],
        assertEquals: parseUnitTestArguments(
          checks[0].substr(firstAssertEqualIndex)
        ),
      });
    }
  });
