...
export class ProblemGeneratorService {
  ...
  async generateProblem(sourceCode: string): Promise<ParsonsSettings> {
    try {
      const response = await axios.post(`${this.apiUrl}/api/problems/generate`, {
        sourceCode
      });
  ...
  private generateSimpleDistractors(codeLines: string[]): string[] {
    ...
    for (const line of codeLines) {
      ...
      if (trimmed.match(/^\s*(if|for|while|def|class|else|elif|try|except|finally|with).*:$/)) {
      ...
      if (varMatches && varMatches.length > 0) {
        for (const varName of varMatches) {
          ...
          if (['if', 'for', 'while', 'def', 'class', 'return', 'print', 'import', 'from'].includes(varName) || 
              varName.length < 3) {
            continue;
          }
          ...
          if (typoName !== varName) {
            const distractor = trimmed.replace(new RegExp(`\\b${varName}\\b`), typoName);
    ...
    const maxDistractors = Math.min(uniqueDistractors.length, Math.ceil(codeLines.length / 2));
